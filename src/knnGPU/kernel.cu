 ...
 * 							where the features of the i-th sample are stored in
 * 							the slice from devSamples[i*numFeatures] to
 * 							devSamples[i*numFeatures + numFeatures]
 ...
 * @param[in,out] result	Pointer to an integer where the total number of
 * 							success will be stored
 */
 __global__ void scoreSolution(void *devSamples, void *devTarget,
	 						   int numFeatures, int numSamples, int* result){
	 ...
	 // Stop execution if the sample id is not in the samples range (necessary
	 // for generalizing the number of blocks and threads)
	 ...
	 // Aux sample with invalid index and  infinite distance for initializing
	 // the K nearest neighbours array.
	 float2 inf;
	 inf.x = -1;
	 inf.y = 99999999;

	 // K (plus one in order to ease the update function) nearest neighbours,
	 // stored as float2, where:
	 // 		x: sample index
	 ...
	 // Computation of distances between this thread sample and the remaining
	 // ones.
	 // TODO: Improve the efficiency of this loop: the matrix of distances is
	 // symmetric, use that!
	 // TODO: Maybe use shared memory to improve efficiency.
	 for(int i=0; i<numSamples; i++){
		 // Leave one out main behaviour: do not consider this sample in order
		 // not to bias the final score
		 if(i == sample){
			 continue;
		 }
		 ...
		 newSample.x = i;
		 newSample.y = computeDistance(myFeatures, globalSamples + i * numFeatures, numFeatures);
	 ...
	 // Populate the classes array with the classes of the K nearest
	 // neighbours.
 ...
 * 							where the features of the i-th test sample are
 * 							stored in the slice from devTest[i*numFeatures] to
 * 							devTest[(i+1)*numFeatures]
 * @param[in]	devTargetTrain Pointer to an array of size numSamples, where
 * 							the i-th value stores the class of the i-th sample.
 ...
 */
 __global__ void scoreOut(void *devSamples, void *devTest,
						  ...
						  int numFeatures, int numSamples, int numTest,
						  int* result){
	 // Pointers to the features, the target and the result: CUDA global memory
	 float* globalSamples = (float*)devSamples;
	 ...
	 for(int i=0; i<numSamples; i++){
		 ...
		 // Check whether this new sample should be in the K nearest
		 // neighbours.
		 updateKNearest(kNearest, newSample);
	 }
